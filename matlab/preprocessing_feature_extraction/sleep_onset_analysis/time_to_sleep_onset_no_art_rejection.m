...
if not(isfolder('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time'))
    mkdir('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time')
end


cd '/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time'
...
for sbj_id = 1:length(cleanMESA_ID)
    ...
    %sbj1 = Patent('patient_original_id', cleanMESA_ID(sbj_id),...
    ...
    sbj2 = period_extract(sbj2,0, 'No Artefact Rejection', 'Keep clean onset');

    sbj2 = sleep_analysis(sbj2,10,36000);
    
    sbj2 = identify_sleep_onset(sbj2, 2, 'N2'); % identify the start of the sleep period and bedtime 

    time2sleeponset = sbj2.time_to_sleep;

    sbj2 = extract_presleep_onset_EEG(sbj2, time2sleeponset, 'No Artefact Rejection', 'Keep clean onset'); % extract continious pre-sleep-onset EEG period 
    
    sbj2 = pre_sleep_onset_EEG_epoching(sbj2, 6, 0, 30,'No Artefact Rejection', 'Keep clean onset'); % break it up into 6 second epochs 


    %% Save the resulting patient files 

    % change the path to a desired folder for saving 
    %if not(isfolder('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/sbj'))
    %    mkdir('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/sbj')
    %end

    %cd '/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/sbj'
...
excluded_sbj_ids = [337, 76];
for sbj_id = 133:length(filtered_sbjs)
    ...
    %cd '/Users/anastasia/Dropbox/sbj examples/sbj2'
    cd '/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/sbj'
    filename = sbj_filename(filtered_sbjs(sbj_id));
    load (filename)
    
    EEG_chs = {'Fz-Cz','Cz-Oz','C4-M1'};
    ifcardiac = 0;
    samp_sbj2 = sample_gen_sleepons_regression(sbj2, EEG_chs, ifcardiac, 'No Artefact Rejection');
    samp_sbj2 = ft_eval_sleep_onset(samp_sbj2,'new_sleep_EEG_ft_dscrp_modified.txt',0);
    disp(['Samples from subject No.',num2str(sbj_id),' have been calculated'])
    % Save the resulting sample files 
     % change the path to a desired folder for saving 

    if not(isfolder('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/unichannel'))
        mkdir('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/unichannel')
    end

    cd '/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/unichannel'

    
    %cd
    %/mnt/FastData/anastasia/sleep_onset/time_to_sleep_regression/30_mins/samp_sbj2/
    filename = sample_filename(filtered_sbjs(sbj_id));
...
for sbj_id = 1: length(cleanMESA_ID)
    ...
    if ~isempty(high_order_fts_sbj2)
        if not(isfolder('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/multichannel'))
            mkdir('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/multichannel')
        end

        cd '/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/multichannel'
...
for sbj_id = 1:length(cleanMESA_ID)
   ...
   % cd  /mnt/FastData/anastasia/sleep_onset/samp_sbj_1min_proof_of_concept/multichannel_ft 
    ...
    % Save the final samples
        ...
        if not(isfolder('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/final'))
            mkdir('/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/final')
        end


        cd '/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/no_artrej_all_time/samp_sbj2/final'
...
writetable(all_sbj_sample_table, 'regression_so_samples_allsbjs_noart_allonset.csv')
