function sampleObj_coll = ft_eval_sleep_onset_without_gamma(sampleObj_coll,txt_feature,ifcardiac, varargin)
...
parfor i = 1:num_samps %iterating through the number of samples 
    ...
    if sum(isnan(y)) == 0
        Fs = samp_now.Fs; 
        [ft,~] = sleep_ft_extract_new(y,Fs, python_directory, 1, 'Min Freq', 0.1, 'Max Freq', 32);        
        ft = struct2cell(ft); 
