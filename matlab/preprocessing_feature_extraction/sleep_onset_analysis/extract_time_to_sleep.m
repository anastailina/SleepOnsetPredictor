...
time2sleeponset = NaN(length(cleanMESA_ID), 1);
for sbj_id = 1: length(cleanMESA_ID)
    disp(['Processing Sbj ', num2str(sbj_id)])
    cd '/mnt/LongTermStorage/anastasia/mesa_analysed/thesis 2023/sleep onset/sbj'
    filename = sbj_filename(cleanMESA_ID(sbj_id));
    load (filename)
    if ~isempty(sbj2.time_to_sleep)
        time2sleeponset(sbj_id) = sbj2.time_to_sleep; 
    end 

end 

cd /home/anastasia/Dropbox
save ("time_to_sleep_onset.mat", 'time2sleeponset')
...
for sbj_id = 82:length(cleanMESA_ID)
